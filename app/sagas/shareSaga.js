...
const shareSaga = function* shareSaga() {
  while (true) {
    ...
    if (yield select(getOffline)) {
      Alert.alert('Offline Mode', "You're offline. Please connect to wi-fi or turn on cellular data to share.", [
        {
          text: 'Settings',
...
const waitUntilSyncedThenShareDictionary = function* waitUntilSyncedThenShareDictionary(dictionary) {
  try {
    yield put({ type: SHOW_SYNC_MODAL })
    while (true) {
      const { single, all } = yield race({
      ...
      if (all) break
      if (single) {
        if ((yield select(getAllPhrases)).filter(e => e.dictionary === dictionary).every(e => e.synced !== false)) {
          break
        }
      }
    }
...
const sharePhrase = function sharePhrase(phrase) {
  ...
  Share.share({ message, title: 'Phrazes', url }, { dialogTitle: 'Share a phraze' }).then(e =>
...
const shareAllPhrases = function shareAllPhrases(user_id) {
  ...
  Share.share({ message, title: 'Phrazes', url }, { dialogTitle: 'Share all your phrazes' }).then(e =>
...
const shareDictionary = function shareDictionary({ user_id, dictionary }) {
  ...
  Share.share({ message, title: 'Phrazes', url }, { dialogTitle: `Share "${dictionary}"` }).then(e =>
    store.dispatch({ type: SHARE_DICTIONARY_COMPLETED }),
  )
  return true
}
